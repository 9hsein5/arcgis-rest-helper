...
const getMyFavItems = (): Promise<AgolItem[]>=>{
    ...
    const { myFavGroupId, token, ArcGISOnlineHost } = defaultOptions;

    return new Promise(async(resolve, reject)=>{

        if(!myFavGroupId || !token){
            reject('skip query my fav items for anonymous user');
        }

        const requestUrl = `${ArcGISOnlineHost}/sharing/rest/search`;

        const params = {
            start: 1,
            num: 1000,
            q: `(group:${myFavGroupId})`,
            f: 'json',
            token
        };
...
const removeFromMyFavGroup = (itemId=''): Promise<ShareOperationResponse>=>{
    ...
    const { myFavGroupId, token } = defaultOptions;
...
const toggleShareAsMyFav = (itemId:string, isAddingToMyFav:boolean): Promise<ShareOperationResponse>=>{
    ...
    const requestUrl = `${ArcGISOnlineHost}/sharing/rest/content/items/${itemId}/${operation}`;

    const bodyFormData = new FormData();
    bodyFormData.append('groups', myFavGroupId);
...
export const toggleShareWithMyFavGroup = (itemId:string): Promise<string[]>=>{
